...
def hartmann_3d_family_numpy(X: np.ndarray, variant_params: dict, device: str = "cpu") -> np.ndarray:
    """计算 Hartmann-3D 变体函数值"""
    x_tensor = torch.from_numpy(X).float().to(device)
    with torch.no_grad():
        y = hartmann_3d_family_torch(x_tensor, **variant_params).cpu().numpy()
    return y.astype(np.float32)


class Hartmann3DVariantFunction(ObjectiveFunction):
    """Hartmann-3D 变体函数"""

    def __init__(self, variant_params: dict, device: str = "cpu"):
        self.variant_params = variant_params
        self._device = device
        self._lower = np.array([0.0, 0.0, 0.0], dtype=np.float32)
        self._upper = np.array([1.0, 1.0, 1.0], dtype=np.float32)
    ...
    def _find_global_min(self) -> float:
        ...
        grid_points = np.stack([X1.flatten(), X2.flatten(), X3.flatten()], axis=1).astype(np.float32)
        ...
        def func(x):
            x = np.array(x, dtype=np.float32).reshape(1, 3)
    ...
    def __call__(self, X: np.ndarray) -> np.ndarray:
        ...
        return hartmann_3d_family_numpy(X, self.variant_params, self._device)
...
# ==================== 改进的奖励函数 ====================
class ImprovedHartmann3DBOEnv:
    """改进的贝叶斯优化环境 - Hartmann-3D 函数"""
    ...
    def reset(self, seed=None):
        ...
        self.current_func = Hartmann3DVariantFunction(variant_params, self.device)
...
# ==================== 训练函数 ====================
def train_improved(
    variants_path: str = "./data/hartmann_3d_family_variants.npz",
    model_path: str = "./model/finetuned_tabpfn_hartmann_3d_family.ckpt",
    total_episodes: int = 5000,  # 增加训练量
    max_steps: int = 20,
    n_candidates: int = 128,
    update_every: int = 20,  # 更频繁的更新
    save_every: int = 500,
    save_dir: str = "./runs/ppo_bo_hartmann_3d",
    seed: int = 42,
    ...
    print(f"\n{'='*70}")
    print(f"Improved PPO Training for Bayesian Optimization (Hartmann-3D)")
    print(f"{'='*70}")
    ...
    print(f"环境已创建: {env.num_variants} 个 Hartmann-3D 变体")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--variants_path", type=str, default="./data/hartmann_3d_family_variants.npz")
    parser.add_argument("--model_path", type=str, default="./model/finetuned_tabpfn_hartmann_3d_family.ckpt")
    ...
    parser.add_argument("--save_dir", type=str, default="./runs/ppo_bo_hartmann_3d")
