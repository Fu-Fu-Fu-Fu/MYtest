...
# ============================================================
# 0. 标准 Hartmann-3D 函数（numpy 版，主要用于检查和对比）
# ============================================================
def hartmann_3d_np(x1: np.ndarray, x2: np.ndarray, x3: np.ndarray) -> np.ndarray:
    """
...
# ============================================================
# 0.1 生成 Hartmann-3D 数据集
# ============================================================
def generate_hartmann_3d_dataset(
    n_samples: int = 10_000,
    x1_range: Tuple[float, float] = (0.0, 1.0),
    x2_range: Tuple[float, float] = (0.0, 1.0),
    x3_range: Tuple[float, float] = (0.0, 1.0),
    save_path: str = "./data/hartmann_3d_dataset.csv",
    seed: int = 42,
) -> None:
    """
    生成基于 Hartmann-3D 函数的微调数据集：
    每一行数据为 x1, x2, x3, y，并保存为 CSV。
    """
    # 确保保存目录存在
    ...
    x3 = rng.uniform(x3_range[0], x3_range[1], size=n_samples).astype(np.float32)

    # 计算 y = hartmann_3d(x1, x2, x3)
    y = hartmann_3d_np(x1, x2, x3)

    # 组装为 DataFrame，列顺序：x1, x2, x3, y
    df = pd.DataFrame({
        "x1": x1,
        "x2": x2,
        "x3": x3,
    ...
    print(f"Saved Hartmann-3D dataset with {n_samples} samples to: {save_path}")
    print(f"   x1 range: [{x1.min():.4f}, {x1.max():.4f}]")
    print(f"   x2 range: [{x2.min():.4f}, {x2.max():.4f}]")
    print(f"   x3 range: [{x3.min():.4f}, {x3.max():.4f}]")
    print(f"   y  range: [{y.min():.4f}, {y.max():.4f}]")
    print("---------------------------\n")


# ============================================================
# 1. Hartmann-3D 变体族（torch 版），用于 similarity 与生成变体函数值
# ============================================================
def hartmann_3d_family_torch(
    x: torch.Tensor,
    dx1: float = 0.0,
    dx2: float = 0.0,
    dx3: float = 0.0,
    ...
    """
    Hartmann-3D 变体族：在输入上做线性变换 (sx, dx)，在输出上做仿射变换 (alpha, beta)。
    x: (..., 3) 的 Tensor，最后一维为 [x1, x2, x3]
    """
    # 支持 (N,3) 或 (B,N,3) 等形状
    x1 = x[..., 0]
    x2 = x[..., 1]
    ...
    # 组合变换后的输入
    X_t = torch.stack([x1_t, x2_t, x3_t], dim=-1)  # (..., 3)
...
def hartmann_3d_family_numpy_from_params(
    X: np.ndarray,
    variant_params: Dict[str, float],
    device: str = "cpu",
) -> np.ndarray:
    """
    方便在 numpy 上调用 hartmann_3d_family_torch：
    X: (N, 3) numpy array
    返回: (N,) numpy array
    """
    x_tensor = torch.from_numpy(X).float().unsqueeze(0).to(device)  # (1, N, 3)
    with torch.no_grad():
        y = hartmann_3d_family_torch(x_tensor, **variant_params).cpu().numpy()[0]
    return y.astype(np.float32)


# ============================================================
# 2. 相似度度量：对齐 Spearman（使用完整3D对齐）
# ============================================================
def find_best_alignment_1d(
    ...
    # 3D 互相关（使用 scipy.signal.correlate 支持任意维度）
...
def calculate_similarity(
    ...
    """
    计算变体与标准 Hartmann-3D 在同一网格上的对齐 Spearman 相关系数。
    """
    x_tensor = torch.tensor(grid_points, dtype=torch.float32, device=device).unsqueeze(0)
    with torch.no_grad():
        y_var = (
            hartmann_3d_family_torch(x_tensor, **variant_params)
            .cpu()
            .numpy()
            .flatten()
            .astype(np.float32)
...
def build_base_grid_and_y(config: dict):
    """
    构造标准 Hartmann-3D 的全局测试网格：
      - X_test_grid: (G^3, 3)
      - X1, X2, X3: (G, G, G) meshgrid
      - y_base: (G^3,) 标准 Hartmann-3D 在该网格上的值
    """
    ...
    y_base = hartmann_3d_np(X_test_grid[:, 0], X_test_grid[:, 1], X_test_grid[:, 2])  # (G^3,)
    ...
    return X_test_grid, X1, X2, X3, y_base


def sample_variant_params(rng: np.random.Generator) -> Dict[str, float]:
    """
    随机采样一组 Hartmann-3D 变体参数。
    Hartmann-3D 定义域为 [0, 1]，值域约为 [-3.86, 0]
    """
    params = {
        # 输入平移（范围较小以保持在定义域内）
        "dx1": float(rng.uniform(-0.1, 0.1)),
        "dx2": float(rng.uniform(-0.1, 0.1)),
        "dx3": float(rng.uniform(-0.1, 0.1)),
        # 输入缩放
        "sx1": float(rng.uniform(0.8, 1.2)),
        "sx2": float(rng.uniform(0.8, 1.2)),
        "sx3": float(rng.uniform(0.8, 1.2)),
        # 输出缩放与平移（Hartmann-3D 值域约为 [-3.86, 0]）
...
def select_hartmann_3d_variants(
    config: dict,
    grid_points: np.ndarray,
    y_base: np.ndarray,
    grid_shape: Tuple[int, int, int],
    ...
    """
    用拒绝采样的方式，选出若干个相似度在 [rho_min, rho_max] 的 Hartmann-3D 变体。
    返回：
      - variants:       长度 num_variants 的参数字典列表
      - variant_y_grids: shape (num_variants, G^3)，每行是一个变体在全局网格上的值
    ...
    while len(variants) < num_variants and trials < max_trials:
        ...
        if rho_min <= rho <= rho_max:
            ...
            with torch.no_grad():
                y_var = (
                    hartmann_3d_family_torch(x_tensor, **params)
                    .cpu()
                    .numpy()
                    .flatten()
                    .astype(np.float32)
    ...
    variant_y_grids = np.stack(variant_y_list, axis=0)  # (num_variants, G^3)
...
# ============================================================
# 4. 为每个变体构造上下文池（基于已有 hartmann_3d_dataset.csv 的 X）
# ============================================================
def load_base_context_points(config: dict) -> Tuple[np.ndarray, np.ndarray]:
    """
    从 hartmann_3d_dataset.csv 中读取基础上下文点：
      - X_ctx_base: (N_ctx_base, 3)
      - y_ctx_base: (N_ctx_base,) —— 标准 Hartmann-3D 在这些点上的值
    """
    data_path = config.get("data_path", "./data/hartmann_3d_dataset.csv")
    # 检查数据集是否存在，不存在则生成
    if not os.path.exists(data_path):
        print(f"Dataset not found at {data_path}, generating...")
        generate_hartmann_3d_dataset(
            n_samples=config.get("num_context_samples", 10_000),
            x1_range=config.get("x1_range", (0.0, 1.0)),
            x2_range=config.get("x2_range", (0.0, 1.0)),
            x3_range=config.get("x3_range", (0.0, 1.0)),
            save_path=data_path,
...
def build_variant_context_pools(
    ...
    for i, params in enumerate(variants):
        y_ctx = hartmann_3d_family_numpy_from_params(X_ctx_base, params, device=device)
...
# ============================================================
class Hartmann3DFamilyMetaDataset(Dataset):
    """
    每个 __getitem__ 返回一个"源任务"（meta-task）：
      - X_ctx:  (n_ctx, 3)   —— 来自某一个变体的上下文点
      - y_ctx:  (n_ctx,)
      - X_test: (n_test, 3)  —— 公共全局网格
...
def demo_family_meta_dataset(
    ...
    """
    num_tasks_demo = 5
    ds = Hartmann3DFamilyMetaDataset(
    ...
    print("--- Demo Hartmann3DFamilyMetaDataset tasks ---")
...
# ============================================================
# 7. 在标准 Hartmann-3D 网格上的简单评估（用于训练过程监控）
# ============================================================
def evaluate_regressor_on_base_hartmann_3d_grid(
    regressor: TabPFNRegressor,
    ...
    """
    在标准 Hartmann-3D 的全局网格上评估当前 TabPFN。
    """
...
# ============================================================
# 8. 微调主循环（在 Hartmann-3D 变体族上 finetune TabPFN）
# ============================================================
def run_finetuning_family(
    ...
    print("--- Start finetuning on Hartmann-3D family ---")
    num_epochs = config["finetuning"]["epochs"]

    for epoch in range(num_epochs + 1):
        # 1) 每个 epoch 前先在标准 Hartmann-3D 网格上评估一次
        mse, mae, r2 = evaluate_regressor_on_base_hartmann_3d_grid(
            regressor,
            regressor_config,
            X_ctx_base,
            y_ctx_base,
        ...
        print(
            f"{status} Evaluation on base Hartmann-3D grid | "
    ...
    save_path = "./model/finetuned_tabpfn_hartmann_3d_family.ckpt"
...
def main():
    ...
    config = {
        ...
        # Hartmann-3D 域 & 网格（定义域为 [0, 1] x [0, 1] x [0, 1]）
        "x1_range": (0.0, 1.0),
        "x2_range": (0.0, 1.0),
        "x3_range": (0.0, 1.0),
        "grid_size": 10,  # 10×10×10 网格

        # 从 CSV 读取的基础上下文池大小（标准 Hartmann-3D）
    ...
    if not os.path.exists(config["data_path"]):
        print("=" * 50)
        print("Step 0: Generating Hartmann-3D dataset...")
        print("=" * 50)
        generate_hartmann_3d_dataset(
            n_samples=config["num_context_samples"],
            x1_range=config["x1_range"],
            x2_range=config["x2_range"],
            x3_range=config["x3_range"],
            save_path=config["data_path"],
            seed=config["random_seed"],
        )

    # --- 步骤 1：构造基础网格 & 标准 Hartmann-3D 网格值 ---
    X_test_grid, X1, X2, X3, y_base_grid = build_base_grid_and_y(config)

    # --- 步骤 2：从 CSV 读取基础上下文点（标准 Hartmann-3D） ---
    ...
    variants, variant_y_grids, variant_rhos = select_hartmann_3d_variants(
        config,
        X_test_grid,
        y_base_grid,
        grid_shape,
    ...
    np.savez(
        "./data/hartmann_3d_family_variants.npz",
    ...
    print("Saved Hartmann-3D family variants to ./data/hartmann_3d_family_variants.npz")
