"""
评估改进版双塔 PPO 策略在 Goldstein-Price 函数上的性能
...
def evaluate_on_goldstein_price(
    model_path: str,
    tabpfn_path: str,
    n_runs: int = 10,
    max_steps: int = 20,
    ...
    """在标准 Goldstein-Price 函数上评估所有方法"""
    os.makedirs(save_dir, exist_ok=True)

    print(f"\n{'='*60}")
    print("Comparison on Goldstein-Price Function (Improved Dual-Tower)")
    print(f"{'='*60}")
    ...
    # 创建 Goldstein-Price 函数
    func = GoldsteinPriceFunction()
    global_min = func.optimal_value
    lower, upper = func.bounds

    print(f"Goldstein-Price global minimum: {global_min:.6f}")
...
def plot_comparison(results: Dict, max_steps: int, save_dir: str):
    ...
    plt.title("Comparison on Goldstein-Price Function (Improved Dual-Tower)", fontsize=14)
...
def plot_trajectories(func, results: Dict, save_dir: str, n_init: int = 2):
    ...
    # 使用 log scale 以更好地显示 Goldstein-Price 函数
    Z_log = np.log10(Z + 1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Improved Dual-Tower RL policy on Goldstein-Price")
    parser.add_argument("--model_path", type=str,
                        default="/gpfs/radev/pi/cohan/yz979/lin/test/Glod_1218/runs/ppo_bo_1219_v1/ppo_ep10000.pt",
                        help="PPO model path")
    parser.add_argument("--tabpfn_path", type=str,
                        default="./model/finetuned_tabpfn_goldstein_price_family.ckpt",
    ...
    results = evaluate_on_goldstein_price(
