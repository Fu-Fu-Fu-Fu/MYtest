...
class RLPolicyTesterCNN:
    ...
    def __init__(self, policy_path, tabpfn_model_path, device='cuda'):
        ...
        # 标准 Ackley 的全局最小值（Ground Truth）
        # 在 (0, 0) 处，值为 0
        self.global_min_y = 0.0
    ...
    def _evaluate(self, x):
        """真值函数：标准 Ackley，直接用物理坐标评估"""
        x = np.asarray(x, dtype=np.float32)
        return float(ackley_np(x[0], x[1]))
    ...
    def run_rl_tabpfn_optimization(self, max_steps=20, seed=None, verbose=False):
        """
        用 TabPFNRegressor（微调后的）在 Ackley 上做优化
        """
        self._set_seed(seed)
        x_history, y_history = [], []
    ...
    def run_gp_rl_optimization(self, max_steps=20, seed=None, verbose=False):
        """
        用 GP 在 Ackley 真值上做 surrogate，在网格上算 (mu, sigma)，
        ...
        for step in range(max_steps - 1):
            ...
            # Ackley 值域约 [0, 23]，调整 kernel 参数
            kernel = C(10.0, (1e-2, 1e3)) * Matern(length_scale=0.5, length_scale_bounds=(1e-2, 1e2), nu=2.5) + \
    ...
    def run_bo_gp_optimization(self, max_steps=20, seed=None, verbose=False):
        ...
        for step in range(max_steps - 1):
            # Ackley 值域约 [0, 23]，调整 kernel 参数
            kernel = C(10.0, (1e-2, 1e3)) * Matern(length_scale=1.0, nu=2.5)
            gp = GaussianProcessRegressor(
                kernel=kernel,
                alpha=1e-6,
                normalize_y=True,
...
def plot_comparison_results(
    ...
    ax.axhline(y=0.0, color='lime', linestyle=':', linewidth=2, label='Global Optimum (0.0)')
    ...
    ax2.axhline(y=0.0, color='lime', linestyle=':', linewidth=2, label='Global Optimum (0.0)')
...
# =============================
# 4. 主入口：在 Ground Truth Ackley 上测试
# =============================

def test_on_ground_truth():
    print("=" * 80)
    print("Test on Ground Truth Ackley (TabPFN + CNN PPO)")
    print("=" * 80)

    # 这里改成你实际的路径
    policy_path = '/gpfs/radev/pi/cohan/yz979/lin/test/Ackley/runs/TabPFN_CNN_PPO_Ackley_1765455168/agent_75.pt'  # 请根据实际修改
    tabpfn_model_path = './model/finetuned_tabpfn_ackley_family.ckpt'
