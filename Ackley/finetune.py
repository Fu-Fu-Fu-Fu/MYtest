...
# ============================================================
# 0. 标准 Ackley 函数（numpy 版，主要用于检查和对比）
# ============================================================
...
# 0.1 生成 Ackley 数据集
# ============================================================
def generate_ackley_dataset(
    n_samples: int = 10_000,
    x1_range: Tuple[float, float] = (-5.0, 5.0),
    x2_range: Tuple[float, float] = (-5.0, 5.0),
    save_path: str = "./data/ackley_dataset.csv",
    seed: int = 42,
) -> None:
    """
    生成基于 Ackley 函数的微调数据集：
    ...
    print(f"✅ Saved Ackley dataset with {n_samples} samples to: {save_path}")
...
# ============================================================
# 1. Ackley 变体族（torch 版），用于 similarity 与生成变体函数值
# ============================================================
...
def ackley_family_numpy_from_params(
    X: np.ndarray,
    variant_params: Dict[str, float],
    device: str = "cpu",
) -> np.ndarray:
    """
    方便在 numpy 上调用 ackley_family_torch：
    X: (N, 2) numpy array
    返回: (N,) numpy array
    """
...
def calculate_similarity(
    ...
    """
    计算变体与标准 Ackley 在同一网格上的对齐 Spearman 相关系数。
    """
    x_tensor = torch.tensor(grid_points, dtype=torch.float32, device=device).unsqueeze(0)
    with torch.no_grad():
        y_var = (
            ackley_family_torch(x_tensor, **variant_params)
            .cpu()
            .numpy()
            .flatten()
            .astype(np.float32)
...
def build_base_grid_and_y(config: dict):
    ...
    构造标准 Ackley 的全局测试网格：
      - X_test_grid: (G^2, 2)
      - X1, X2: (G, G) meshgrid
      - y_base: (G^2,) 标准 Ackley 在该网格上的值
    """
    x1_min, x1_max = config.get("x1_range", (-5.0, 5.0))
    x2_min, x2_max = config.get("x2_range", (-5.0, 5.0))
    ...
    y_base = ackley_np(X_test_grid[:, 0], X_test_grid[:, 1])  # (G^2,)
...
def sample_variant_params(rng: np.random.Generator) -> Dict[str, float]:
    """
    随机采样一组 Ackley 变体参数。
    Ackley 定义域为 [-5, 5]，值域约为 [0, 23]
    """
    params = {
        # 输入平移
        "dx1": float(rng.uniform(-1.0, 1.0)),
        "dx2": float(rng.uniform(-1.0, 1.0)),
        # 输入缩放
        "sx1": float(rng.uniform(0.8, 1.2)),
        "sx2": float(rng.uniform(0.8, 1.2)),
        # 输出缩放与平移（Ackley 值域约为 [0, 23]）
        "alpha": float(rng.uniform(0.8, 1.2)),
        "beta": float(rng.uniform(-5.0, 5.0)),
    }
    return params


def select_ackley_variants(
    config: dict,
    grid_points: np.ndarray,
    y_base: np.ndarray,
    grid_shape: Tuple[int, int],
    ...
    """
    用拒绝采样的方式，选出若干个相似度在 [rho_min, rho_max] 的 Ackley 变体。
    返回：
    ...
    while len(variants) < num_variants and trials < max_trials:
        ...
        if rho_min <= rho <= rho_max:
            ...
            with torch.no_grad():
                y_var = (
                    ackley_family_torch(x_tensor, **params)
                    .cpu()
                    .numpy()
                    .flatten()
                    .astype(np.float32)
...
# ============================================================
# 4. 为每个变体构造上下文池（基于已有 ackley_dataset.csv 的 X）
# ============================================================
def load_base_context_points(config: dict) -> Tuple[np.ndarray, np.ndarray]:
    """
    从 ackley_dataset.csv 中读取基础上下文点：
      - X_ctx_base: (N_ctx_base, 2)
      - y_ctx_base: (N_ctx_base,) —— 标准 Ackley 在这些点上的值
    """
    data_path = config.get("data_path", "./data/ackley_dataset.csv")
    # 检查数据集是否存在，不存在则生成
    if not os.path.exists(data_path):
        print(f"⚠️ Dataset not found at {data_path}, generating...")
        generate_ackley_dataset(
            n_samples=config.get("num_context_samples", 10_000),
            x1_range=config.get("x1_range", (-5.0, 5.0)),
            x2_range=config.get("x2_range", (-5.0, 5.0)),
            save_path=data_path,
...
def build_variant_context_pools(
    ...
    for i, params in enumerate(variants):
        y_ctx = ackley_family_numpy_from_params(X_ctx_base, params, device=device)
...
# ============================================================
class AckleyFamilyMetaDataset(Dataset):
    """
...
def demo_family_meta_dataset(
    ...
    print("--- Demo AckleyFamilyMetaDataset tasks ---")
...
def make_ackley_family_splitter(
    X_ctx_pools: List[np.ndarray],
    y_ctx_pools: List[np.ndarray],
    X_test_grid: np.ndarray,
    y_test_grids: np.ndarray,
...
def create_finetuning_dataloader_family(
    ...
    splitter = make_ackley_family_splitter(
        X_ctx_pools,
        y_ctx_pools,
        X_test_grid,
        y_test_grids,
...
# ============================================================
# 7. 在标准 Ackley 网格上的简单评估（用于训练过程监控）
# ============================================================
def evaluate_regressor_on_base_ackley_grid(
    regressor: TabPFNRegressor,
...
# ============================================================
# 8. 微调主循环（在 Ackley 变体族上 finetune TabPFN）
# ============================================================
def run_finetuning_family(
    ...
    print("--- Start finetuning on Ackley family ---")
    num_epochs = config["finetuning"]["epochs"]

    for epoch in range(num_epochs + 1):
        # 1) 每个 epoch 前先在标准 Ackley 网格上评估一次
        mse, mae, r2 = evaluate_regressor_on_base_ackley_grid(
            regressor,
            regressor_config,
            X_ctx_base,
            y_ctx_base,
        ...
        print(
            f"📊 {status} Evaluation on base Ackley grid | "
    ...
    save_path = "./model/finetuned_tabpfn_ackley_family.ckpt"
...
def main():
    ...
    config = {
        ...
        # Ackley 域 & 网格（定义域为 [-5, 5] x [-5, 5]）
        "x1_range": (-5.0, 5.0),
        "x2_range": (-5.0, 5.0),
        "grid_size": 20,  # 20×20 网格

        # 从 CSV 读取的基础上下文池大小（标准 Ackley）
        "data_path": "./data/ackley_dataset.csv",
    ...
    if not os.path.exists(config["data_path"]):
        print("=" * 50)
        print("Step 0: Generating Ackley dataset...")
        print("=" * 50)
        generate_ackley_dataset(
    ...
    variants, variant_y_grids, variant_rhos = select_ackley_variants(
        config,
        X_test_grid,
        y_base_grid,
        grid_shape,
    ...
    np.savez(
        "./data/ackley_family_variants.npz",
    ...
    print("Saved Ackley family variants to ./data/ackley_family_variants.npz")
    ...
    # --- 步骤 5（路径 B）：构造 AckleyFamilyMetaDataset，打印几个 task 看看 ---
