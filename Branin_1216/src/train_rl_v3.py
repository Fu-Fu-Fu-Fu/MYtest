...
class ImprovedBraninBOEnv:
    ...
    def _compute_improved_reward(
        ...
        """
        # 定义"成功"的标准
        # 对于 Branin 这类函数，Regret < 1e-2 通常就算找到了
        # SUCCESS_THRESHOLD = 0.5
        
        # current_regret = self.best_y - self.global_min
        # # old_regret = old_best - self.global_min
        
        # # === 情况 A: 任务完成 (Success) ===
        # if current_regret < SUCCESS_THRESHOLD:
        #     # 巨大奖励：假设 max_steps=20，每步惩罚约 -1，
        #     # 成功奖励要足够大，让 Agent 觉得尽早成功是划算的
        #     return 5.0
        # step_penalty = -0.1
        # log_regret = np.log10(current_regret + 1e-6)
        # regret_penalty = -log_regret
        # reward = step_penalty + regret_penalty
