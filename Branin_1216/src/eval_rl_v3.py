"""
评估改进版双塔 PPO 策略在 Branin 函数上的性能 (v3)
对比方法：TabPFN+RL(v3), GP+RL(v3), GP+EI, TabPFN+EI
"""
...
class RLAgentV3:
    ...
    def __init__(self, model_path: str, coord_dim: int = 2, hidden_dim: int = 128,
        ...
        self.policy.eval()
        print(f"已加载改进版双塔模型 (v3): {model_path}")
...
def run_gp_rl_v3(func, agent, X_context: np.ndarray, y_context: np.ndarray,
...
def evaluate_on_branin(
    ...
    print(f"\n{'='*60}")
    print("Comparison on Branin Function (v3 Improved Dual-Tower)")
    print(f"{'='*60}")
    print(f"Methods: TabPFN+RL(v3), GP+RL(v3), GP+EI, TabPFN+EI, Random")
    ...
    # 存储结果
    results = {
        "TabPFN+RL(v3)": {"regrets": [], "trajectories": []},
        "GP+RL(v3)": {"regrets": [], "trajectories": []},
    ...
    for run in range(n_runs):
        ...
        regrets, traj = run_tabpfn_rl_v3(func, agent, tabpfn_path, X_init, y_init,
                                          max_steps, rng1, device, n_candidates)
        results["TabPFN+RL(v3)"]["regrets"].append(regrets)
        results["TabPFN+RL(v3)"]["trajectories"].append(traj)
        print(f"  TabPFN+RL(v3): final regret = {regrets[-1]:.6f}")

        # GP + RL(v3)
        rng2 = np.random.default_rng(run_seed)
        regrets, traj = run_gp_rl_v3(func, agent, X_init, y_init, max_steps, rng2, n_candidates)
        results["GP+RL(v3)"]["regrets"].append(regrets)
        results["GP+RL(v3)"]["trajectories"].append(traj)
        print(f"  GP+RL(v3): final regret = {regrets[-1]:.6f}")
...
def plot_comparison(results: Dict, max_steps: int, save_dir: str):
    ...
    plt.title("Comparison on Branin Function (Improved Dual-Tower v3)", fontsize=14)
...
def plot_trajectories(func, results: Dict, save_dir: str, n_init: int = 2):
    ...
    methods = ["TabPFN+RL(v3)", "GP+RL(v3)", "GP+EI", "TabPFN+EI", "Random"]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Improved Dual-Tower RL policy (v3) on Branin")
    parser.add_argument("--model_path", type=str,
                        default="/gpfs/radev/pi/cohan/yz979/lin/test/Branin_1216/runs/ppo_bo_1216v3_2/ppo_ep5000.pt",
                        help="PPO model path")
    ...
    parser.add_argument("--save_dir", type=str, default="./results_v3", help="Save directory")
    # 网络参数 (v3 默认值)
