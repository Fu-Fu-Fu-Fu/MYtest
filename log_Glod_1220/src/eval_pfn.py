...
# ---- Log-Goldstein-Price & 数据准备 ----
def log_goldstein_price(x1: np.ndarray, x2: np.ndarray) -> np.ndarray:
    """
...
def prepare_data_for_eval(config: dict):
    df = pd.read_csv(config.get("data_path", "./data/log_goldstein_price_dataset.csv"))
    ...
    y_test_grid = log_goldstein_price(X_test_grid[:, 0], X_test_grid[:, 1])
...
def eval_and_plot_for_context_sizes(
    ...
    for k in context_sizes:
        ...
        # ========== 3) 高斯过程回归(GP) ==========
        # Log-Goldstein-Price 值域约为 [1.1, 13.8]，适当调整 kernel 参数
        kernel = (
            C(1.0, (1e-3, 1e3))
        ...
        axes[0].set_title(f"True Log-Goldstein-Price (k={k})")
        ...
        plt.suptitle(f"Log-Goldstein-Price predictions with {k} context points", fontsize=14)

        save_path = f"./figs/log_goldstein_price_contours_k{k}_with_gp.png"
...
def main():
    config = {
        ...
        # Log-Goldstein-Price 定义域为 [-2, 2] x [-2, 2]
        "x1_range": (-2.0, 2.0),
        "x2_range": (-2.0, 2.0),
        "grid_size": 20,
    ...
    tuned_regressor = TabPFNRegressor(
        model_path="./model/finetuned_tabpfn_log_goldstein_price_family.ckpt",
        device=config["device"],
        n_estimators=1,
        random_state=config["random_seed"],
        inference_precision=torch.float32,
