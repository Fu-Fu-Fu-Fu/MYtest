"""
评估改进版双塔 PPO 策略在 Log-Goldstein-Price 函数上的性能
...
from train_rl import (
    ...
    LogGoldsteinPriceVariantFunction,
)
...
# ==================== 标准 Log-Goldstein-Price 函数 ====================
class LogGoldsteinPriceFunction(ObjectiveFunction):
    """
    Log-Goldstein-Price 测试函数
    定义域: x1, x2 ∈ [-2, 2]
...
def evaluate_on_log_goldstein_price(
    model_path: str,
    tabpfn_path: str,
    n_runs: int = 10,
    max_steps: int = 20,
    ...
    """在标准 Log-Goldstein-Price 函数上评估所有方法"""
    os.makedirs(save_dir, exist_ok=True)

    print(f"\n{'='*60}")
    print("Comparison on Log-Goldstein-Price Function (Improved Dual-Tower)")
    print(f"{'='*60}")
    ...
    # 创建 Log-Goldstein-Price 函数
    func = LogGoldsteinPriceFunction()
    global_min = func.optimal_value
    lower, upper = func.bounds

    print(f"Log-Goldstein-Price global minimum: {global_min:.6f}")
...
def plot_comparison(results: Dict, max_steps: int, save_dir: str):
    ...
    plt.title("Comparison on Log-Goldstein-Price Function (Improved Dual-Tower)", fontsize=14)
...
def plot_trajectories(func, results: Dict, save_dir: str, n_init: int = 2):
    ...
    # Log-Goldstein-Price 函数已经取了对数，直接使用
    Z_log = Z

    methods = ["TabPFN+RL", "GP+RL", "GP+EI", "TabPFN+EI", "Random"]

    for ax, method in zip(axes, methods):
        contour = ax.contourf(X1, X2, Z_log, levels=50, cmap='viridis', alpha=0.8)
        ax.contour(X1, X2, Z_log, levels=20, colors='white', alpha=0.3, linewidths=0.5)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Improved Dual-Tower RL policy on Log-Goldstein-Price")
    parser.add_argument("--model_path", type=str,
                        default="/gpfs/radev/pi/cohan/yz979/lin/test/log_Glod_1220/runs/ppo_bo_1220/ppo_ep5000.pt",
                        help="PPO model path")
    parser.add_argument("--tabpfn_path", type=str,
                        default="./model/finetuned_tabpfn_log_goldstein_price_family.ckpt",
    ...
    results = evaluate_on_log_goldstein_price(
