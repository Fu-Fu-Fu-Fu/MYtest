...
# ============================================================
# 0. 标准 Log-Goldstein-Price 函数（numpy 版，主要用于检查和对比）
# ============================================================
def log_goldstein_price_np(x1: np.ndarray, x2: np.ndarray) -> np.ndarray:
    """
    标准 2D Log-Goldstein-Price 函数，定义域通常为 x1, x2 ∈ [-2, 2]
    全局最小值在 (0, -1)，值为 log(3) ≈ 1.0986

    Goldstein-Price:
    f(x1, x2) = [1 + (x1 + x2 + 1)^2 * (19 - 14*x1 + 3*x1^2 - 14*x2 + 6*x1*x2 + 3*x2^2)] *
                [30 + (2*x1 - 3*x2)^2 * (18 - 32*x1 + 12*x1^2 + 48*x2 - 36*x1*x2 + 27*x2^2)]

    Log-Goldstein-Price: log(f(x1, x2))
    """
    # 第一部分
    ...
    gp = term1 * term2
    # 取对数
    return np.log(gp).astype(np.float32)


# ============================================================
# 0.1 生成 Log-Goldstein-Price 数据集
# ============================================================
def generate_log_goldstein_price_dataset(
    n_samples: int = 10_000,
    x1_range: Tuple[float, float] = (-2.0, 2.0),
    x2_range: Tuple[float, float] = (-2.0, 2.0),
    save_path: str = "./data/log_goldstein_price_dataset.csv",
    seed: int = 42,
) -> None:
    """
    生成基于 Log-Goldstein-Price 函数的微调数据集：
    ...
    # 计算 y = log_goldstein_price(x1, x2)
    y = log_goldstein_price_np(x1, x2)
    ...
    print(f"Saved Log-Goldstein-Price dataset with {n_samples} samples to: {save_path}")
...
# ============================================================
# 1. Log-Goldstein-Price 变体族（torch 版），用于 similarity 与生成变体函数值
# ============================================================
def log_goldstein_price_family_torch(
    x: torch.Tensor,
    dx1: float = 0.0,
    dx2: float = 0.0,
    sx1: float = 1.0,
    ...
    """
    Log-Goldstein-Price 变体族：在输入上做线性变换 (sx, dx)，在输出上做仿射变换 (alpha, beta)。
...
def log_goldstein_price_family_numpy_from_params(
    X: np.ndarray,
    variant_params: Dict[str, float],
    device: str = "cpu",
) -> np.ndarray:
    """
    方便在 numpy 上调用 log_goldstein_price_family_torch：
    X: (N, 2) numpy array
    返回: (N,) numpy array
    """
    x_tensor = torch.from_numpy(X).float().unsqueeze(0).to(device)  # (1, N, 2)
    with torch.no_grad():
        y = log_goldstein_price_family_torch(x_tensor, **variant_params).cpu().numpy()[0]
...
def calculate_similarity(
    ...
    """
    计算变体与标准 Log-Goldstein-Price 在同一网格上的对齐 Spearman 相关系数。
    """
    x_tensor = torch.tensor(grid_points, dtype=torch.float32, device=device).unsqueeze(0)
    with torch.no_grad():
        y_var = (
            log_goldstein_price_family_torch(x_tensor, **variant_params)
            .cpu()
            .numpy()
            .flatten()
            .astype(np.float32)
...
def build_base_grid_and_y(config: dict):
    """
    构造标准 Log-Goldstein-Price 的全局测试网格：
      - X_test_grid: (G^2, 2)
      - X1, X2: (G, G) meshgrid
      - y_base: (G^2,) 标准 Log-Goldstein-Price 在该网格上的值
    """
    ...
    y_base = log_goldstein_price_np(X_test_grid[:, 0], X_test_grid[:, 1])  # (G^2,)
...
def sample_variant_params(rng: np.random.Generator) -> Dict[str, float]:
    """
    随机采样一组 Log-Goldstein-Price 变体参数。
    Log-Goldstein-Price 定义域为 [-2, 2]，值域约为 [log(3), log(10^6)] ≈ [1.1, 13.8]
    """
    params = {
        # 输入平移（范围较小以保持在定义域内）
        ...
        # 输出缩放与平移（Log-Goldstein-Price 值域约为 [1.1, 13.8]）
...
def select_log_goldstein_price_variants(
    config: dict,
    grid_points: np.ndarray,
    y_base: np.ndarray,
    grid_shape: Tuple[int, int],
    ...
    """
    用拒绝采样的方式，选出若干个相似度在 [rho_min, rho_max] 的 Log-Goldstein-Price 变体。
    返回：
    ...
    print(
        f"--- Select Log-Goldstein-Price variants ---\n"
    ...
    while len(variants) < num_variants and trials < max_trials:
        ...
        if rho_min <= rho <= rho_max:
            ...
            with torch.no_grad():
                y_var = (
                    log_goldstein_price_family_torch(x_tensor, **params)
                    .cpu()
                    .numpy()
                    .flatten()
                    .astype(np.float32)
...
# ============================================================
# 4. 为每个变体构造上下文池（基于已有 log_goldstein_price_dataset.csv 的 X）
# ============================================================
def load_base_context_points(config: dict) -> Tuple[np.ndarray, np.ndarray]:
    """
    从 log_goldstein_price_dataset.csv 中读取基础上下文点：
      - X_ctx_base: (N_ctx_base, 2)
      - y_ctx_base: (N_ctx_base,) —— 标准 Log-Goldstein-Price 在这些点上的值
    """
    data_path = config.get("data_path", "./data/log_goldstein_price_dataset.csv")
    # 检查数据集是否存在，不存在则生成
    if not os.path.exists(data_path):
        print(f"Dataset not found at {data_path}, generating...")
        generate_log_goldstein_price_dataset(
...
def build_variant_context_pools(
    ...
    for i, params in enumerate(variants):
        y_ctx = log_goldstein_price_family_numpy_from_params(X_ctx_base, params, device=device)
...
# ============================================================
class LogGoldsteinPriceFamilyMetaDataset(Dataset):
    """
...
def demo_family_meta_dataset(
    ...
    """
    num_tasks_demo = 5
    ds = LogGoldsteinPriceFamilyMetaDataset(
    ...
    print("--- Demo LogGoldsteinPriceFamilyMetaDataset tasks ---")
...
def make_log_goldstein_price_family_splitter(
    X_ctx_pools: List[np.ndarray],
    y_ctx_pools: List[np.ndarray],
    X_test_grid: np.ndarray,
    y_test_grids: np.ndarray,
...
def create_finetuning_dataloader_family(
    ...
    splitter = make_log_goldstein_price_family_splitter(
        X_ctx_pools,
        y_ctx_pools,
        X_test_grid,
        y_test_grids,
...
# ============================================================
# 7. 在标准 Log-Goldstein-Price 网格上的简单评估（用于训练过程监控）
# ============================================================
def evaluate_regressor_on_base_log_goldstein_price_grid(
    regressor: TabPFNRegressor,
    ...
    """
    在标准 Log-Goldstein-Price 的全局网格上评估当前 TabPFN。
    """
...
# ============================================================
# 8. 微调主循环（在 Log-Goldstein-Price 变体族上 finetune TabPFN）
# ============================================================
def run_finetuning_family(
    ...
    print("--- Start finetuning on Log-Goldstein-Price family ---")
    num_epochs = config["finetuning"]["epochs"]

    for epoch in range(num_epochs + 1):
        # 1) 每个 epoch 前先在标准 Log-Goldstein-Price 网格上评估一次
        mse, mae, r2 = evaluate_regressor_on_base_log_goldstein_price_grid(
            regressor,
            regressor_config,
            X_ctx_base,
            y_ctx_base,
        ...
        print(
            f"{status} Evaluation on base Log-Goldstein-Price grid | "
    ...
    save_path = "./model/finetuned_tabpfn_log_goldstein_price_family.ckpt"
...
def main():
    ...
    config = {
        ...
        # Log-Goldstein-Price 域 & 网格（定义域为 [-2, 2] x [-2, 2]）
        "x1_range": (-2.0, 2.0),
        "x2_range": (-2.0, 2.0),
        "grid_size": 20,  # 20×20 网格

        # 从 CSV 读取的基础上下文池大小（标准 Log-Goldstein-Price）
        "data_path": "./data/log_goldstein_price_dataset.csv",
    ...
    if not os.path.exists(config["data_path"]):
        print("=" * 50)
        print("Step 0: Generating Log-Goldstein-Price dataset...")
        print("=" * 50)
        generate_log_goldstein_price_dataset(
    ...
    # --- 步骤 1：构造基础网格 & 标准 Log-Goldstein-Price 网格值 ---
    X_test_grid, X1, X2, y_base_grid = build_base_grid_and_y(config)

    # --- 步骤 2：从 CSV 读取基础上下文点（标准 Log-Goldstein-Price） ---
    ...
    variants, variant_y_grids, variant_rhos = select_log_goldstein_price_variants(
        config,
        X_test_grid,
        y_base_grid,
        grid_shape,
    ...
    np.savez(
        "./data/log_goldstein_price_family_variants.npz",
    ...
    print("Saved Log-Goldstein-Price family variants to ./data/log_goldstein_price_family_variants.npz")
    ...
    # --- 步骤 5（路径 B）：构造 LogGoldsteinPriceFamilyMetaDataset，打印几个 task 看看 ---
