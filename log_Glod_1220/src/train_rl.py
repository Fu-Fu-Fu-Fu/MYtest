"""
改进版 PPO 训练 - 针对 Log-Goldstein-Price 函数
...
def log_goldstein_price_family_torch(
    ...
    """
    Log-Goldstein-Price 函数族（支持变体参数）

    标准定义域: x1, x2 ∈ [-2, 2]
    全局最小值: log(f(0, -1)) = log(3) ≈ 1.0986
    """
    x1 = x[..., 0]
    x2 = x[..., 1]
...
def log_goldstein_price_family_numpy(X: np.ndarray, variant_params: dict, device: str = "cpu") -> np.ndarray:
    """计算 Log-Goldstein-Price 变体函数值"""
    x_tensor = torch.from_numpy(X).float().to(device)
    with torch.no_grad():
        y = log_goldstein_price_family_torch(x_tensor, **variant_params).cpu().numpy()
    return y.astype(np.float32)


class LogGoldsteinPriceVariantFunction(ObjectiveFunction):
    """Log-Goldstein-Price 变体函数"""
    ...
    def __call__(self, X: np.ndarray) -> np.ndarray:
        ...
        return log_goldstein_price_family_numpy(X, self.variant_params, self._device)
...
# ==================== 改进的奖励函数 ====================
class ImprovedLogGoldsteinPriceBOEnv:
    """改进的贝叶斯优化环境 - Log-Goldstein-Price 函数"""
    ...
    def reset(self, seed=None):
        ...
        self.current_func = LogGoldsteinPriceVariantFunction(variant_params, self.device)
...
# ==================== 训练函数 ====================
def train_improved(
    variants_path: str = "./data/log_goldstein_price_family_variants.npz",
    model_path: str = "./model/finetuned_tabpfn_log_goldstein_price_family.ckpt",
    total_episodes: int = 5000,  # 增加训练量
    max_steps: int = 20,
    n_candidates: int = 128,
    update_every: int = 20,  # 更频繁的更新
    save_every: int = 500,
    save_dir: str = "./runs/ppo_bo_log_goldstein_price",
    seed: int = 42,
    ...
    print(f"\n{'='*70}")
    print(f"Improved PPO Training for Bayesian Optimization (Log-Goldstein-Price)")
    print(f"{'='*70}")
    ...
    # 创建环境
    env = ImprovedLogGoldsteinPriceBOEnv(
        variants_path=variants_path,
        model_path=model_path,
        max_steps=max_steps,
        n_candidates=n_candidates,
    ...
    print(f"环境已创建: {env.num_variants} 个 Log-Goldstein-Price 变体")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--variants_path", type=str, default="./data/log_goldstein_price_family_variants.npz")
    parser.add_argument("--model_path", type=str, default="./model/finetuned_tabpfn_log_goldstein_price_family.ckpt")
    ...
    parser.add_argument("--save_dir", type=str, default="./runs/ppo_bo_log_goldstein_price")
