...
class CNNAgent(nn.Module):
    ...
    def get_deterministic_action(self, x):
        """
        不采样，直接用 actor_mean 的 tanh，当作确定性动作。
        x: (B, 2, G, G)
        返回: (B, 2)
        """
        feat = self.fc(self.network(x))
...
class RLPolicyTesterCNN:
    ...
    def _tabpfn_maps(self, X_ctx, y_ctx):
        ...
        self.tabpfn_rl.fit(X_fit, y_fit)
        ...
        mean_map = mean.reshape(self.grid_size, self.grid_size).astype(np.float32)
        ...
        if criterion is not None and logits is not None:
            ...
            var = var * (y_std ** 2)
            uncert = var.reshape(self.grid_size, self.grid_size).astype(np.float32)
        else:
            from sklearn.metrics import pairwise_distances
            dists = pairwise_distances(self.grid_points, X_ctx)
            min_dist = dists.min(axis=1)
            uncert = min_dist.reshape(self.grid_size, self.grid_size).astype(np.float32)

        return mean_map, uncert


    def _maps_to_obs(self, mean_map, uncert_map):
        """修复版：使用 robust 归一化，与训练时一致"""
        def robust_normalize(arr):
            p5, p95 = np.percentile(arr, [5, 95])
            range_val = p95 - p5
            if range_val < 1e-6:
                range_val = 1.0
            return (arr - p5) / range_val - 0.5
    ...
    def run_rl_tabpfn_optimization(self, max_steps=20, seed=None, verbose=False):
        ...
        for step in range(max_steps - 1):
            ...
            obs = self._maps_to_obs(mean_map, uncert_map)
...
def test_on_ground_truth():
    ...
    # 这里改成你实际的路径
    policy_path = '/gpfs/radev/pi/cohan/yz979/lin/test/runs/TabPFN_CNN_PPO_1765356019/agent_5.pt'
    ...
    for i in range(num_runs):
        ...
        seed = 42 + i
        print(f"Run {i+1}/{num_runs} (seed={seed})", end='\r')

        # 1. TabPFN + RL
        x, y = tester.run_rl_tabpfn_optimization(max_steps=max_steps, seed=seed, verbose=False)
        rl_res.append({'y_history': y, 'best_y': y.min()})

        # 2. GP + RL
        x, y = tester.run_gp_rl_optimization(max_steps=max_steps, seed=seed, verbose=False)
        gp_rl_res.append({'y_history': y, 'best_y': y.min()})
        print(
            f"run {i}: "
            f"max |TabPFN+RL - GP+RL| = "
            f"{np.max(np.abs(rl_res[i]['y_history'] - gp_rl_res[i]['y_history'])):.4e}"
        )
        print("TabPFN+RL first run y:", rl_res[0]['y_history'][:5])
        print("GP+RL     first run y:", gp_rl_res[0]['y_history'][:5])
