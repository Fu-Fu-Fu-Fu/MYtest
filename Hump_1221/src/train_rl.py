"""
改进版 PPO 训练 - 针对 Six-Hump Camel 函数
...
def six_hump_camel_family_torch(
    ...
    """
    Six-Hump Camel 函数族（支持变体参数）

    标准定义域: x1 ∈ [-3, 3], x2 ∈ [-2, 2]
    全局最小值: f(0.0898, -0.7126) = f(-0.0898, 0.7126) ≈ -1.0316
    """
    x1 = x[..., 0]
    x2 = x[..., 1]
...
def six_hump_camel_family_numpy(X: np.ndarray, variant_params: dict, device: str = "cpu") -> np.ndarray:
    """计算 Six-Hump Camel 变体函数值"""
    x_tensor = torch.from_numpy(X).float().to(device)
    with torch.no_grad():
        y = six_hump_camel_family_torch(x_tensor, **variant_params).cpu().numpy()
    return y.astype(np.float32)


class SixHumpCamelVariantFunction(ObjectiveFunction):
    """Six-Hump Camel 变体函数"""

    def __init__(self, variant_params: dict, device: str = "cpu"):
        self.variant_params = variant_params
        self._device = device
        self._lower = np.array([-3.0, -2.0], dtype=np.float32)
        self._upper = np.array([3.0, 2.0], dtype=np.float32)
    ...
    def __call__(self, X: np.ndarray) -> np.ndarray:
        ...
        return six_hump_camel_family_numpy(X, self.variant_params, self._device)
...
# ==================== 改进的奖励函数 ====================
class ImprovedSixHumpCamelBOEnv:
    """改进的贝叶斯优化环境 - Six-Hump Camel 函数"""
    ...
    def reset(self, seed=None):
        ...
        self.current_func = SixHumpCamelVariantFunction(variant_params, self.device)
...
# ==================== 训练函数 ====================
def train_improved(
    variants_path: str = "./data/six_hump_camel_family_variants.npz",
    model_path: str = "./model/finetuned_tabpfn_six_hump_camel_family.ckpt",
    total_episodes: int = 5000,  # 增加训练量
    max_steps: int = 20,
    n_candidates: int = 128,
    update_every: int = 20,  # 更频繁的更新
    save_every: int = 500,
    save_dir: str = "./runs/ppo_bo_six_hump_camel",
    seed: int = 42,
    ...
    print(f"\n{'='*70}")
    print(f"Improved PPO Training for Bayesian Optimization (Six-Hump Camel)")
    print(f"{'='*70}")
    ...
    print(f"环境已创建: {env.num_variants} 个 Six-Hump Camel 变体")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--variants_path", type=str, default="./data/six_hump_camel_family_variants.npz")
    parser.add_argument("--model_path", type=str, default="./model/finetuned_tabpfn_six_hump_camel_family.ckpt")
    ...
    parser.add_argument("--save_dir", type=str, default="./runs/ppo_bo_six_hump_camel")
