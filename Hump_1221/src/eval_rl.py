"""
评估改进版双塔 PPO 策略在 Six-Hump Camel 函数上的性能
...
class SixHumpCamelFunction(ObjectiveFunction):
    ...
    def bounds(self) -> Tuple[np.ndarray, np.ndarray]:
        lower = np.array([-3.0, -2.0], dtype=np.float32)
        upper = np.array([3.0, 2.0], dtype=np.float32)
        return lower, upper

    @property
    def optimal_value(self) -> float:
        return -1.0316284534898774  # 精确值
...
def evaluate_on_six_hump_camel(
    model_path: str,
    tabpfn_path: str,
    n_runs: int = 10,
    max_steps: int = 20,
    ...
    """在标准 Six-Hump Camel 函数上评估所有方法"""
    os.makedirs(save_dir, exist_ok=True)

    print(f"\n{'='*60}")
    print("Comparison on Six-Hump Camel Function (Improved Dual-Tower)")
    print(f"{'='*60}")
    ...
    # 创建 Six-Hump Camel 函数
    func = SixHumpCamelFunction()
    global_min = func.optimal_value
    lower, upper = func.bounds

    print(f"Six-Hump Camel global minimum: {global_min:.6f}")
...
def plot_comparison(results: Dict, max_steps: int, save_dir: str):
    ...
    plt.title("Comparison on Six-Hump Camel Function (Improved Dual-Tower)", fontsize=14)
...
def plot_trajectories(func, results: Dict, save_dir: str, n_init: int = 2):
    ...
    for ax, method in zip(axes, methods):
        ...
        # 全局最优点 (0.0898, -0.7126) 和 (-0.0898, 0.7126)
        optima = np.array([[0.0898, -0.7126], [-0.0898, 0.7126]])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Improved Dual-Tower RL policy on Six-Hump Camel")
    parser.add_argument("--model_path", type=str,
                        default="/gpfs/radev/pi/cohan/yz979/lin/test/Hump_1221/runs/ppo_bo_1221/ppo_ep1000.pt",
                        help="PPO model path")
    parser.add_argument("--tabpfn_path", type=str,
                        default="./model/finetuned_tabpfn_six_hump_camel_family.ckpt",
    ...
    results = evaluate_on_six_hump_camel(
