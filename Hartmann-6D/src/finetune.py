...
# ============================================================
# 0. 标准 Hartmann-6D 函数（numpy 版，主要用于检查和对比）
# ============================================================
def hartmann_6d_np(X: np.ndarray) -> np.ndarray:
    """
...
# ============================================================
# 0.1 生成 Hartmann-6D 数据集
# ============================================================
def generate_hartmann_6d_dataset(
    n_samples: int = 10_000,
    x_ranges: List[Tuple[float, float]] = None,
    save_path: str = "./data/hartmann_6d_dataset.csv",
    seed: int = 42,
) -> None:
    """
    生成基于 Hartmann-6D 函数的微调数据集：
    每一行数据为 x1, x2, x3, x4, x5, x6, y，并保存为 CSV。
    """
    if x_ranges is None:
        x_ranges = [(0.0, 1.0)] * 6
    ...
    for i in range(6):
        X[:, i] = rng.uniform(x_ranges[i][0], x_ranges[i][1], size=n_samples).astype(np.float32)

    # 计算 y = hartmann_6d(X)
    y = hartmann_6d_np(X)

    # 组装为 DataFrame，列顺序：x1, x2, x3, x4, x5, x6, y
    df = pd.DataFrame({
        "x1": X[:, 0],
        "x2": X[:, 1],
        "x3": X[:, 2],
    ...
    print(f"Saved Hartmann-6D dataset with {n_samples} samples to: {save_path}")
    for i in range(6):
        print(f"   x{i+1} range: [{X[:, i].min():.4f}, {X[:, i].max():.4f}]")
    print(f"   y  range: [{y.min():.4f}, {y.max():.4f}]")
    print("---------------------------\n")


# ============================================================
# 1. Hartmann-6D 变体族（torch 版），用于 similarity 与生成变体函数值
# ============================================================
def hartmann_6d_family_torch(
    x: torch.Tensor,
    dx1: float = 0.0, dx2: float = 0.0, dx3: float = 0.0,
    dx4: float = 0.0, dx5: float = 0.0, dx6: float = 0.0,
    sx1: float = 1.0, sx2: float = 1.0, sx3: float = 1.0,
    sx4: float = 1.0, sx5: float = 1.0, sx6: float = 1.0,
    alpha: float = 1.0,
    beta: float = 0.0,
) -> torch.Tensor:
    """
    Hartmann-6D 变体族：在输入上做线性变换 (sx, dx)，在输出上做仿射变换 (alpha, beta)。
    x: (..., 6) 的 Tensor，最后一维为 [x1, x2, x3, x4, x5, x6]
    """
    # 输入平移 + 缩放
    x_t = []
    ...
    for i in range(6):
        x_t.append(sx[i] * x[..., i] + dx[i])

    # Hartmann-6D 参数
    alpha_h = torch.tensor([1.0, 1.2, 3.0, 3.2], dtype=x.dtype, device=x.device)
    A = torch.tensor([
        [10, 3, 17, 3.5, 1.7, 8],
        [0.05, 10, 17, 0.1, 8, 14],
        [3, 3.5, 1.7, 10, 17, 8],
    ...
    # 组合变换后的输入
    X_t = torch.stack(x_t, dim=-1)  # (..., 6)

    # 计算 Hartmann-6D
    result = torch.zeros_like(x[..., 0])
    for i in range(4):
        diff = X_t - P[i]  # (..., 6)
        inner = torch.sum(A[i] * diff**2, dim=-1)  # (...)
        result = result - alpha_h[i] * torch.exp(-inner)
...
def hartmann_6d_family_numpy_from_params(
    X: np.ndarray,
    variant_params: Dict[str, float],
    device: str = "cpu",
) -> np.ndarray:
    """
    方便在 numpy 上调用 hartmann_6d_family_torch：
    X: (N, 6) numpy array
    返回: (N,) numpy array
    """
    x_tensor = torch.from_numpy(X).float().unsqueeze(0).to(device)  # (1, N, 6)
    with torch.no_grad():
        y = hartmann_6d_family_torch(x_tensor, **variant_params).cpu().numpy()[0]
    return y.astype(np.float32)


# ============================================================
# 2. 相似度度量：对齐 Spearman（使用完整6D对齐）
# ============================================================
def find_best_alignment_1d(
    ...
    # 多维互相关（使用 scipy.signal.correlate 支持任意维度）
    correlation = correlate(Z_source_norm, Z_target_norm, mode="same")
...
def calculate_similarity(
    ...
    """
    计算变体与标准 Hartmann-6D 在同一网格上的对齐 Spearman 相关系数。
    """
    x_tensor = torch.tensor(grid_points, dtype=torch.float32, device=device).unsqueeze(0)
    with torch.no_grad():
        y_var = (
            hartmann_6d_family_torch(x_tensor, **variant_params)
            .cpu()
            .numpy()
            .flatten()
            .astype(np.float32)
...
def build_base_grid_and_y(config: dict):
    """
    构造标准 Hartmann-6D 的全局测试网格：
      - X_test_grid: (G^6, 6)
      - y_base: (G^6,) 标准 Hartmann-6D 在该网格上的值
    """
    grid_size = config.get("grid_size", 5)  # 6D用较小的网格以避免内存问题

    # 获取每个维度的范围
    x_ranges = []
    for i in range(6):
        x_min, x_max = config.get(f"x{i+1}_range", (0.0, 1.0))
        x_ranges.append(np.linspace(x_min, x_max, grid_size, dtype=np.float32))

    # 创建6D网格
    grids = np.meshgrid(*x_ranges, indexing='ij')

    # 展平并组合
    X_test_grid = np.stack([g.ravel() for g in grids], axis=1).astype(np.float32)
    y_base = hartmann_6d_np(X_test_grid)  # (G^6,)
    ...
    return X_test_grid, grids, y_base


def sample_variant_params(rng: np.random.Generator) -> Dict[str, float]:
    """
    随机采样一组 Hartmann-6D 变体参数。
    Hartmann-6D 定义域为 [0, 1]^6，值域约为 [-3.32, 0]
    """
    params = {}
    # 输入平移（范围较小以保持在定义域内）
    for i in range(1, 7):
        params[f"dx{i}"] = float(rng.uniform(-0.1, 0.1))
    # 输入缩放
    for i in range(1, 7):
        params[f"sx{i}"] = float(rng.uniform(0.8, 1.2))
    # 输出缩放与平移（Hartmann-6D 值域约为 [-3.32, 0]）
    params["alpha"] = float(rng.uniform(0.8, 1.2))
    params["beta"] = float(rng.uniform(-0.5, 0.5))
    return params


def select_hartmann_6d_variants(
    config: dict,
    grid_points: np.ndarray,
    y_base: np.ndarray,
    grid_shape: Tuple[int, ...],
    ...
    """
    用拒绝采样的方式，选出若干个相似度在 [rho_min, rho_max] 的 Hartmann-6D 变体。
    返回：
      - variants:       长度 num_variants 的参数字典列表
      - variant_y_grids: shape (num_variants, G^6)，每行是一个变体在全局网格上的值
    ...
    while len(variants) < num_variants and trials < max_trials:
        ...
        if rho_min <= rho <= rho_max:
            ...
            with torch.no_grad():
                y_var = (
                    hartmann_6d_family_torch(x_tensor, **params)
                    .cpu()
                    .numpy()
                    .flatten()
                    .astype(np.float32)
    ...
    variant_y_grids = np.stack(variant_y_list, axis=0)  # (num_variants, G^6)
...
# ============================================================
# 4. 为每个变体构造上下文池（基于已有 hartmann_6d_dataset.csv 的 X）
# ============================================================
def load_base_context_points(config: dict) -> Tuple[np.ndarray, np.ndarray]:
    """
    从 hartmann_6d_dataset.csv 中读取基础上下文点：
      - X_ctx_base: (N_ctx_base, 6)
      - y_ctx_base: (N_ctx_base,) —— 标准 Hartmann-6D 在这些点上的值
    """
    data_path = config.get("data_path", "./data/hartmann_6d_dataset.csv")
    # 检查数据集是否存在，不存在则生成
    if not os.path.exists(data_path):
        print(f"Dataset not found at {data_path}, generating...")
        x_ranges = []
        for i in range(6):
            x_ranges.append(config.get(f"x{i+1}_range", (0.0, 1.0)))
        generate_hartmann_6d_dataset(
...
def build_variant_context_pools(
    ...
    for i, params in enumerate(variants):
        y_ctx = hartmann_6d_family_numpy_from_params(X_ctx_base, params, device=device)
...
# ============================================================
class Hartmann6DFamilyMetaDataset(Dataset):
    """
    每个 __getitem__ 返回一个"源任务"（meta-task）：
      - X_ctx:  (n_ctx, 6)   —— 来自某一个变体的上下文点
      - y_ctx:  (n_ctx,)
      - X_test: (n_test, 6)  —— 公共全局网格
...
def demo_family_meta_dataset(
    ...
    """
    num_tasks_demo = 5
    ds = Hartmann6DFamilyMetaDataset(
    ...
    print("--- Demo Hartmann6DFamilyMetaDataset tasks ---")
    for batch_idx, batch in enumerate(loader):
        ...
        print("  Sample context points (x1, x2, x3, y):")
        print(np.concatenate([X_ctx[:3], y_ctx[:3, None]], axis=1))

        print("  Sample test grid points (x1, x2, x3, y):")
...
def make_hartmann_3d_family_splitter(
    X_ctx_pools: List[np.ndarray],
    y_ctx_pools: List[np.ndarray],
    X_test_grid: np.ndarray,
    y_test_grids: np.ndarray,
...
def create_finetuning_dataloader_family(
    ...
    splitter = make_hartmann_3d_family_splitter(
        X_ctx_pools,
        y_ctx_pools,
        X_test_grid,
        y_test_grids,
...
# ============================================================
# 7. 在标准 Hartmann-6D 网格上的简单评估（用于训练过程监控）
# ============================================================
def evaluate_regressor_on_base_hartmann_6d_grid(
    regressor: TabPFNRegressor,
    ...
    """
    在标准 Hartmann-6D 的全局网格上评估当前 TabPFN。
    """
...
# ============================================================
# 8. 微调主循环（在 Hartmann-6D 变体族上 finetune TabPFN）
# ============================================================
def run_finetuning_family(
    ...
    print("--- Start finetuning on Hartmann-6D family ---")
    num_epochs = config["finetuning"]["epochs"]

    for epoch in range(num_epochs + 1):
        # 1) 每个 epoch 前先在标准 Hartmann-6D 网格上评估一次
        mse, mae, r2 = evaluate_regressor_on_base_hartmann_6d_grid(
            regressor,
            regressor_config,
            X_ctx_base,
            y_ctx_base,
        ...
        print(
            f"{status} Evaluation on base Hartmann-6D grid | "
    ...
    save_path = "./model/finetuned_tabpfn_hartmann_6d_family.ckpt"
...
def main():
    ...
    config = {
        ...
        "x5_range": (0.0, 1.0),
        "x6_range": (0.0, 1.0),
        "grid_size": 5,  # 5^6 = 15625 网格点（6D用较小网格）

        # 从 CSV 读取的基础上下文池大小（标准 Hartmann-6D）
    ...
    if not os.path.exists(config["data_path"]):
        print("=" * 50)
        print("Step 0: Generating Hartmann-6D dataset...")
        print("=" * 50)
        x_ranges = [(config[f"x{i}_range"]) for i in range(1, 7)]
        generate_hartmann_6d_dataset(
    ...
    # --- 步骤 1：构造基础网格 & 标准 Hartmann-6D 网格值 ---
    X_test_grid, grids, y_base_grid = build_base_grid_and_y(config)

    # --- 步骤 2：从 CSV 读取基础上下文点（标准 Hartmann-6D） ---
    X_ctx_base, y_ctx_base = load_base_context_points(config)

    # --- 步骤 3：在基础网格上选 32 个相似度在 [0.4,0.6] 的变体 ---
    grid_shape = tuple([config["grid_size"]] * 6)  # (G, G, G, G, G, G)
    variants, variant_y_grids, variant_rhos = select_hartmann_6d_variants(
        config,
        X_test_grid,
        y_base_grid,
        grid_shape,
    ...
    np.savez(
        "./data/hartmann_6d_family_variants.npz",
    ...
    print("Saved Hartmann-6D family variants to ./data/hartmann_6d_family_variants.npz")
    ...
    # --- 步骤 5（路径 B）：构造 Hartmann3DFamilyMetaDataset，打印几个 task 看看 ---
