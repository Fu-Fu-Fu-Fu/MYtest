...
class SphereFunction(ObjectiveFunction):
    ...
    def __init__(self, dim: int = 3, bound: float = 5.0):
        self._dim = dim
        self._bound = bound
...
class RastriginFunction(ObjectiveFunction):
    ...
    def __init__(self, dim: int = 3, bound: float = 5.12):
        self._dim = dim
        self._bound = bound
...
def main():
    ...
    # 方式1：使用 Hartmann-6D 函数
    print("\n" + "=" * 60)
    print("示例 1: Hartmann-6D 函数")
    print("=" * 60)

    hartmann_func = Hartmann6DFunction()

    # 手动指定上下文点（10个随机点）
    rng = np.random.default_rng(42)
    lower, upper = hartmann_func.bounds
    X_context = rng.uniform(lower, upper, size=(10, hartmann_func.dim)).astype(np.float32)
    ...
    # print("\n" + "=" * 60)
    # print("示例 2: Sphere 函数 (6D)")
    # print("=" * 60)
    #
    # sphere_func = SphereFunction(dim=6, bound=5.0)
