...
# ============================================================
# 0. 标准 Branin 函数（numpy 版，主要用于检查和对比）
# ============================================================
def branin_np(x1: np.ndarray, x2: np.ndarray) -> np.ndarray:
    """
    标准 Branin 函数，定义域通常为 x1 ∈ [-5, 10], x2 ∈ [0, 15]
    """
    a = 1.0
    b = 5.1 / (4.0 * np.pi ** 2)
    c = 5.0 / np.pi
...
# ============================================================
# 1. Branin 变体族（torch 版），用于 similarity 与生成变体函数值
# ============================================================
def branin_family_torch(
    ...
    """
    Branin 变体族：在输入上做线性变换 (sx, dx)，在输出上做仿射变换 (alpha, beta)。
    同时保留 a,b,c,r,s,t 参数在默认值（你如果想以后可以再扰动这些参数）。
...
def branin_family_numpy_from_params(
    ...
    """
    方便在 numpy 上调用 branin_family_torch：
    X: (N, 2) numpy array
    返回: (N,) numpy array
    """
...
def calculate_similarity(
    ...
    """
    计算变体与标准 Branin 在同一网格上的对齐 Spearman 相关系数。
    """
    x_tensor = torch.tensor(grid_points, dtype=torch.float32, device=device).unsqueeze(0)
    with torch.no_grad():
        y_var = (
            branin_family_torch(x_tensor, **variant_params)
            .cpu()
            .numpy()
            .flatten()
            .astype(np.float32)
...
def build_base_grid_and_y(config: dict):
    ...
    y_base = branin_np(X_test_grid[:, 0], X_test_grid[:, 1])  # (G^2,)
...
def sample_variant_params(rng: np.random.Generator) -> Dict[str, float]:
    """
    随机采样一组 Branin 变体参数。
    范围可以后续根据实际情况调整，这里给一个比较温和的扰动范围。
    """
    params = {
        # 输入平移
        "dx1": float(rng.uniform(-2.0, 2.0)),
        "dx2": float(rng.uniform(-2.0, 2.0)),
        # 输入缩放
        "sx1": float(rng.uniform(0.7, 1.3)),
        "sx2": float(rng.uniform(0.7, 1.3)),
        # 输出缩放与平移
        "alpha": float(rng.uniform(0.7, 1.3)),
        "beta": float(rng.uniform(-20.0, 20.0)),
        # 下方 a,b,c,r,s,t 保持默认，后续如果你想再加扰动可以放开
        # "a": ...,
        # "b": ...,
        # ...
    }
...
def select_branin_variants(
    ...
    """
    用拒绝采样的方式，选出若干个相似度在 [rho_min, rho_max] 的 Branin 变体。
    返回：
    ...
    while len(variants) < num_variants and trials < max_trials:
        ...
        if rho_min <= rho <= rho_max:
            ...
            with torch.no_grad():
                y_var = (
                    branin_family_torch(x_tensor, **params)
                    .cpu()
                    .numpy()
                    .flatten()
                    .astype(np.float32)
...
def build_variant_context_pools(
    ...
    for i, params in enumerate(variants):
        y_ctx = branin_family_numpy_from_params(X_ctx_base, params, device=device)
        X_ctx_pools.append(X_ctx_base.copy())  # 坐标共用一份也可以，这里 copy 一份更明确
...
# ============================================================
class BraninFamilyMetaDataset(Dataset):
    """
    ...
    注意：这里只是一个“原始任务”表示，方便你直观检查 task；
    真正用于 TabPFN 微调的预处理仍然通过 regressor.get_preprocessed_datasets 完成。
    """
...
def demo_family_meta_dataset(
    ...
    print("--- Demo BraninFamilyMetaDataset tasks ---")
...
def make_branin_family_splitter(
    X_ctx_pools: List[np.ndarray],
    y_ctx_pools: List[np.ndarray],
    X_test_grid: np.ndarray,
    y_test_grids: np.ndarray,
    ...
    每次调用 splitter(X_all, y_all) 都会：
      1) 随机选择一个变体 v；
      2) 在该变体的上下文池中抽取 n_ctx ∈ [min_context, max_context] 个点；
      3) test 使用公共网格 + 对应变体在网格上的 y。

    注意：X_all, y_all 在这里完全不使用，只是 get_preprocessed_datasets 内部需要一个“驱动”数据量。
    """
...
def create_finetuning_dataloader_family(
    ...
    splitter = make_branin_family_splitter(
        X_ctx_pools,
        y_ctx_pools,
        X_test_grid,
        y_test_grids,
    ...
    # 这里 get_preprocessed_datasets 的 X_all, y_all 仅用于决定生成多少个 meta-task。
    # 具体的上下文与 test 由 splitter 决定，与 y_ctx_base 本身无关。
...
# ============================================================
# 7. 在标准 Branin 网格上的简单评估（用于训练过程监控）
# ============================================================
def evaluate_regressor_on_base_branin_grid(
    regressor: TabPFNRegressor,
    ...
    """
    在标准 Branin 的全局网格上评估当前 TabPFN。
    这里的上下文点只用标准 Branin 的一小部分点。
    """
...
# ============================================================
# 8. 微调主循环（在 Branin 变体族上 finetune TabPFN）
# ============================================================
def run_finetuning_family(
    ...
    print("--- Start finetuning on Branin family ---")
    num_epochs = config["finetuning"]["epochs"]

    for epoch in range(num_epochs + 1):
        # 1) 每个 epoch 前先在标准 Branin 网格上评估一次
        mse, mae, r2 = evaluate_regressor_on_base_branin_grid(
            regressor,
            regressor_config,
            X_ctx_base,
            y_ctx_base,
        ...
        print(
            f"📊 {status} Evaluation on base Branin grid | "
        ...
        for data_batch in progress_bar:
            ...
            loss_fn = znorm_space_bardist_[0]  # 内部带了反归一化
    ...
    # 训练结束后，保存整个 TabPFN 模型（包括微调后的 backbone）
    os.makedirs("./model", exist_ok=True)
    save_path = "./model/finetuned_tabpfn_branin_family.ckpt"
...
# ============================================================
# 9. main：串起来步骤 0–5（不做 contour 的离线 eval）
# ============================================================
def main():
    # 全局配置（可以根据需要调整）
    config = {
        ...
        "max_variant_trials": 32 * 300,  # 最多尝试这么多随机变体
        ...
        "finetuning": {
            ...
            "meta_batch_size": 1,  # 目前必须 1
            "max_data_size": 20,   # 决定每个 epoch 产生多少个 meta-task
        },
    }

    # --- 步骤 0：构造基础网格 & 标准 Branin 网格值 ---
    X_test_grid, X1, X2, y_base_grid = build_base_grid_and_y(config)

    # --- 步骤 1：从 CSV 读取基础上下文点（标准 Branin） ---
    X_ctx_base, y_ctx_base = load_base_context_points(config)

    # --- 步骤 2：在基础网格上选 32 个相似度在 [0.4,0.6] 的变体 ---
    grid_shape = (config["grid_size"], config["grid_size"])
    variants, variant_y_grids, variant_rhos = select_branin_variants(
        config,
        X_test_grid,
        y_base_grid,
        grid_shape,
    ...
    np.savez(
        "./data/branin_family_variants.npz",
    ...
    print("Saved Branin family variants to ./data/branin_family_variants.npz")
    ...
    # --- 步骤 4（路径 B）：构造 BraninFamilyMetaDataset，打印几个 task 看看 ---
    ...
    # --- 步骤 5：TabPFN 初始化 + finetuning dataloader + 微调 ---
